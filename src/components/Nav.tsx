...
export default function Nav() {
  ...
  return (
    ...
    >
      <nav className="container-x flex h-[68px] items-center justify-between">
        ...
        <div className="hidden items-center gap-1 md:flex">
          {NAV_LINKS.map((l) => (
            <a
              key={l.href}
              href={l.href}
              className="rounded-lg px-3.5 py-2 text-[14px] text-[var(--muted)] transition-colors hover:text-[var(--text)]"
            >
              {l.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-2.5">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="btn btn-ghost px-4! py-2! text-[14px]! hidden sm:inline-flex"
          >
          ...
          <a href="#install" className="btn btn-primary px-4! py-2! text-[14px]!">
            Get started
          </a>
          <button
            aria-label="Menu"
            className="md:hidden ml-1 grid h-9 w-9 place-items-center rounded-lg border border-[var(--border-strong)]"
            onClick={() => setOpen((v) => !v)}
          >
      ...
      {open && (
        <div className="md:hidden container-x pb-4">
          <div className="glass flex flex-col p-2">
            {NAV_LINKS.map((l) => (
              <a
                key={l.href}
                href={l.href}
